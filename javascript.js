...
var icon = "set";
// Nested arrays containing the winning tile combinations
...
// Function takes the value of the button pressed in HTML file
// This sets wether you play against computer or friend
function toggle(a) {
    against = a;
...
function select(id, sq) {
    ...
    if ( get === "") {
		...
		// Finds the tile that has just been clicked in aiTiles array
        a = aiTiles.indexOf(sq);
		// Removes the used tile from aiTiles array
        aiTiles.splice(a, 1);
        test();
        switchTurn();
    if (against == "c") {
...
function computer() {
	...
	// Variable i holds length of tiles left in the array of unused board tiles
    i = aiTiles.length;
	// Generate random number from 0 - Number of tiles left
    x = Math.floor(Math.random() * i);
	// Picks random tile from array using the index generated by Math.random
    block = aiTiles[x];
	// Chooses the game board tile
    d = box[block];
...
function test() {
    ...
    // For loop to check how many tiles are already taken up
		...
		// Moves through winning tiles array and checks if each winning tile combination is
		// taken up by the same icon, nought or cross
